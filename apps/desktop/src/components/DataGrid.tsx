...
export default function DataGrid({ tab }: { tab: Tab }) {
  ...
  // Content width of the scroll container — `clientWidth` excludes the
  // vertical scrollbar, so the filler stops exactly at the scrollbar edge
  // instead of either overshooting (gap under the scrollbar) or leaving a
  // strip of bare scroll area on the right.
  ...
  /** Last row the user interacted with — gets a soft accent stripe so
   *  the eye doesn't lose its place between an edit click and the Save
   *  button at the bottom. Cleared on successful save. */
   ...
   *  the row-level "dirty" stripe. Recomputed when pendingEdits
   *  changes; cheap because pendingCount is typically tiny. */
  ...
  // editable gates inline cell edits, +Row, Save All. Read-only mode
  // (toggled via the StatusBar lock) flips this off without changing
  // anything on the Postgres side — it's a UI guard.
  const editable = pk.length > 0 && !!tab.sourceRelation && !isReadOnly;
  ...
  // Track the scroll container's content width so the filler col can
  // stretch the rows to the right edge. `clientWidth` excludes the
  // vertical scrollbar, so the filler stops exactly at the scrollbar.
  //
  // useLayoutEffect (not useEffect) so the first measurement lands BEFORE
  // the browser paints — otherwise the initial frame renders with
  // contentW=0 → fillerW=0 and the row rules visibly stop at the last
  // column until a later resize nudges a re-measure. The ResizeObserver
  // keeps it correct on container resize and scrollbar appear/disappear.
  useLayoutEffect(() => {
  ...
  // fill the viewport, so row rules + hover stripes reach the right edge.
  //
  // We give it an EXPLICIT width rather than `width:auto`: the data rows
  // render as `position:absolute` tables whose `width:100%` shrink-to-fits
  // to their content under `table-layout:fixed`, collapsing an auto filler
  // col to 0 (the bug where horizontal lines stopped after the last
  // column). An explicit px width is honoured regardless.
  const fillerW = Math.max(0, contentW - totalColW);
  // The tables are sized to exactly cover the columns + filler. Using a
  ...
  const dataVirtualCount = Math.max(0, displayOrder.length - pinned.length);
  /** Draft rows are virtualized too — they live at indices
   *  [dataVirtualCount, dataVirtualCount + pendingInserts.length). The
   *  virtualizer doesn't know the difference; the renderer below
   *  branches on the index. */
  const virtualCount = dataVirtualCount + pendingInserts.length;
  ...
  const discardAllEdits = () => updateTab(tab.id, { pendingEdits: {}, pendingInserts: [] });

  const setDraftCell = (draftIdx: number, colIdx: number, value: string) => {
    const next = pendingInserts.map((r, i) =>
      i === draftIdx ? { ...r, [colIdx]: value } : r);
    updateTab(tab.id, { pendingInserts: next });
  };
  const clearDraftCell = (draftIdx: number, colIdx: number) => {
    const next = pendingInserts.map((r, i) => {
      if (i !== draftIdx) return r;
      const copy = { ...r };
      delete copy[colIdx];
      return copy;
    });
  ...
  const discardDraftRow = (draftIdx: number) => {
    updateTab(tab.id, { pendingInserts: pendingInserts.filter((_, i) => i !== draftIdx) });
  };
  ...
  const saveAllEdits = async () => {
    ...
    try {
      ...
      // Append INSERT statements for every draft row. Cells the user
      // left untouched are omitted so the DB default (or NULL) kicks
      // in — that's how a NOT NULL column with a default works without
      // making the user type the value. Rows with no fields at all
      // become INSERT … DEFAULT VALUES, same as the old +Row.
      const qq = (s: string) => `"${s.replace(/"/g, '""')}"`;
      const schemaIdent = qq(tab.sourceRelation.schema);
      const tableIdent  = qq(tab.sourceRelation.table);
      for (const draft of pendingInserts) {
        const colIdxs = Object.keys(draft)
          .map((k) => Number(k))
          .filter((k) => Number.isFinite(k) && cols[k]);
        if (colIdxs.length === 0) {
          stmts.push(`INSERT INTO ${schemaIdent}.${tableIdent} DEFAULT VALUES;`);
          continue;
        }
        const colList = colIdxs.map((i) => qq(cols[i].name)).join(", ");
        const valList = colIdxs.map((i) => {
          const v = draft[i];
          if (v === "" || v === "__NULL__") return "NULL";
          return valueLiteral(v, cols[i].type_name);
        }).join(", ");
        stmts.push(`INSERT INTO ${schemaIdent}.${tableIdent} (${colList}) VALUES (${valList});`);
      }
      stmts.push("COMMIT;");
      for (const sql of stmts) {
        await api.execute(tab.connId, sql);
      }
      // Clear pending + force a refresh of the data so the user sees
      // the committed values (both updates and inserts).
      updateTab(tab.id, {
        pendingEdits: {},
        pendingInserts: [],
  ...
  /** Reusable <colgroup> so header + body line up exactly. The filler col
   *  is always present and has no fixed width — under `table-layout: fixed`
   *  with the table stretched to `width: 100%`, it absorbs whatever space
   *  the real columns don't, so row rules + hover stripes always reach the
   *  right edge regardless of scrollbar width or measurement timing. */
  const colgroup = (
    <colgroup>
  ...
  return (
    <>
      ...
      <div ref={parentRef} className="grid-scroll">
          ...
          <tbody>
            {pinned.map((rowIdx) => (
              <Row
                ...
                onCellClick={(r, c) => { setFocusedRow(r); onCellClick(r, c); }}
                hasFiller={fillerW > 0}
                editing={editing}
                editable={editable}
                pendingEdits={pendingEdits}
                rowClassName={[
                  focusedRow === rowIdx ? "row-focused" : "",
                  dirtyRows.has(rowIdx) ? "row-dirty" : "",
                ].filter(Boolean).join(" ") || undefined}
                onStartEdit={(r, c) => { setFocusedRow(r); setEditing({ row: r, col: c }); }}
                onCommitEdit={(r, c, v) => {
                  setEditing(null);
            ...
            <tr style={{ height: bodyTotalH }} aria-hidden="true">
              <td colSpan={cols.length + 1} style={{ padding: 0, border: 0 }}>
                <div className="virtual-pad" style={{ height: bodyTotalH, position: "relative" }}>
                  {items.map((vi) => {
                    // Branch: draft rows live past dataVirtualCount.
                    // Their visual position uses (vi.index) for zebra
                    // so the pattern stays continuous with data rows.
                    const zebra = vi.index % 2 === 0 ? "row-even" : "row-odd";
                    if (vi.index >= dataVirtualCount) {
                      const draftIdx = vi.index - dataVirtualCount;
                      const draft = pendingInserts[draftIdx] ?? {};
                      return (
                        <table
                          key={`d-${draftIdx}`}
                          className={`grid grid-virt-row ${zebra} row-draft`}
                          style={{
                            position: "absolute",
                            top: vi.start,
                            left: 0,
                        ...
                        >
                          ...
                          <tbody>
                            <DraftRow
                              ...
                              hasFiller={fillerW > 0}
                              editing={editing}
                              onStartEdit={(c) => setEditing({ row: -1 - draftIdx, col: c })}
                              onCommitEdit={(c, v) => {
                                setEditing(null);
                                if (v === "") clearDraftCell(draftIdx, c);
                                else setDraftCell(draftIdx, c, v);
                              }}
                              onCancelEdit={() => setEditing(null)}
                              onDiscard={() => discardDraftRow(draftIdx)}
                            />
                          </tbody>
                        </table>
                      );
                    ...
                    const isDirtyRow = dirtyRows.has(rowIdx);
                    const rowCls = [
                      "grid",
                      "grid-virt-row",
                      zebra,
                      isFocused ? "row-focused" : "",
                      isDirtyRow ? "row-dirty" : "",
                    ].filter(Boolean).join(" ");
                    return (
                      <table
                        key={`r-${rowIdx}`}
            ...
            {/* Tail filler — extends vertical column rules + zebra
                stripes below the last data row, all the way down to
                the bottom of the viewport. Rendered as N empty rows
                of ROW_H height rather than one tall row so the zebra
                pattern is continuous (Excel-style empty grid). The
                final partial row absorbs any leftover pixels. */}
            {renderedH > bodyTotalH && (() => {
              const fillerH = renderedH - bodyTotalH;
              const fullRows = Math.floor(fillerH / ROW_H);
              const leftover = fillerH - fullRows * ROW_H;
              const startIdx = rows.length; // continue zebra from last data row
              const out: React.ReactNode[] = [];
              for (let i = 0; i < fullRows; i++) {
                const zebra = (startIdx + i) % 2 === 0 ? "row-even" : "row-odd";
                out.push(
                  <tr key={`f-${i}`} style={{ height: ROW_H }} className={`grid-filler ${zebra}`}>
              ...
              if (leftover > 0) {
                out.push(
                  <tr key="f-tail" style={{ height: leftover }} className="grid-filler">
                    <td className="grid-gutter" />
                    {cols.map((c) => <td key={c.name} />)}
                    {fillerW > 0 && <td className="grid-filler-cell" aria-hidden="true" />}
                  </tr>,
        ...
        {/* Empty state: the grid itself already renders headers +
            zebra-striped filler rows (Excel-style empty canvas), so a
            "no rows" placeholder would be redundant clutter. We only
            show a hint when there are no columns at all — i.e. before
            the first query has run on a fresh tab. */}
        {rows.length === 0 && cols.length === 0 && !tab.running && (
          <div className="grid-empty-hint">
            <span className="muted">Run a query to populate the grid.</span>
          </div>
        )}
      </div>
...
/**
 * One draft row — appended via +Row, lives only in pendingInserts
 * until Save All commits it. Visually distinguishes itself with a
 * row-draft class (subtle green wash) and shows "—" in the gutter
 * (no real row number yet). Per-cell edits write straight back to
 * pendingInserts via the parent's setDraftCell handler.
 */
function DraftRow({
  draftIdx, cols, draft, hasFiller, editing,
  onStartEdit, onCommitEdit, onCancelEdit, onDiscard,
}: {
  draftIdx: number;
  ...
  onCommitEdit: (col: number, value: string) => void;
  onCancelEdit: () => void;
  onDiscard: () => void;
}) {
  // Editing key: negative rowIdx encodes draft index so the same
  // `editing` state can carry both kinds of cells without collision.
  const editingCol = editing?.row === -1 - draftIdx ? editing.col : null;
  return (
    <tr>
      <td className="grid-gutter draft-gutter" title="Draft row — Save All to commit">
      ...
      {cols.map((c, ci) => {
        const v = draft[ci] ?? "";
        const isEditing = editingCol === ci;
        const isSet = draft[ci] !== undefined;
        return (
          <td
            key={ci}
            className={`draft-cell ${isSet ? "dirty" : ""} ${isEditing ? "editing" : ""}`}
            onClick={() => onStartEdit(ci)}
            title={isSet ? `Pending: ${v}` : "Click to edit · empty cell uses the column's default"}
          >
            {isEditing ? (
              <input
                className="cell-edit-input"
                autoFocus
                defaultValue={v === "__NULL__" ? "" : v}
                onBlur={(e) => onCommitEdit(ci, e.currentTarget.value)}
                onKeyDown={(e) => {
                  if (e.key === "Enter") onCommitEdit(ci, (e.target as HTMLInputElement).value);
                  else if (e.key === "Escape") { e.preventDefault(); onCancelEdit(); }
                }}
              />
            ) : isSet ? (
              <span>{v === "__NULL__" ? "NULL" : v}</span>
            ) : (
              <span className="muted" style={{ fontStyle: "italic" }}>(default)</span>
            )}
          </td>
        );
      })}
...
function Row({
  ...
  editing, editable, pendingEdits, rowClassName,
  ...
  /** Extra classes to mark the row (focused / dirty). Used by the
   *  pinned-rows path where the parent table isn't per-row. */
  rowClassName?: string;

...
export interface Tab {
  ...
  /** Draft rows the user has appended via +Row. Each entry maps
   *  colIdx → user-typed string (or "__NULL__" for an explicit NULL).
   *  Missing column entries are left out of the INSERT statement so
   *  the column's DB default applies. Saved in the same transaction
   *  as pendingEdits on Save All. */
  pendingInserts?: Record<number, string>[];
...
interface State {
  ...
  /** When a session is flagged read-only, UI surfaces that would
   *  mutate the DB (inline cell edits, +Row, Save edits, object-op
   *  Apply, etc.) should refuse. The lock icon in the StatusBar
   *  toggles this flag. Backend execution still allows SELECT —
   *  this is a *frontend guard rail*, not a Postgres role change. */
  ...
  copySource: {
    ...
    schema?: string;
    table?: string;
    /** Pre-select a target session in the CopyDialog. Set by the
     *  Inspector's per-connection direct-copy entries so the user
     *  doesn't have to pick the destination again. */
    targetConnId?: string;
  } | null;
  ...
  /** Toggle (or explicitly set) a session's read-only flag. */
  setReadOnly: (id: string, ro: boolean) => void;
...
export const useStore = create<State>((set, get) => ({
  ...
  setReadOnly: (id, ro) => set((st) => ({ readOnly: { ...st.readOnly, [id]: ro } })),

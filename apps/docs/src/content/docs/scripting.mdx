...
Scripting is **SQL-engine only** (PostgreSQL / MySQL / SQLite). The
toggle is hidden for MongoDB and Redis.
...
(SQL → JavaScript); the run button becomes **Run Script**.
...
- **`console.log(...)`** output is captured and shown in the **Message**
  tab.

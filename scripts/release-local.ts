...
async function checkVersionConsistency(expected: string): Promise<void> {
  const cfg = JSON.parse(await readFile(join(REPO, "src-tauri", "tauri.conf.json"), "utf8"));
  ...
  const cargo = await readFile(join(REPO, "src-tauri", "Cargo.toml"), "utf8");
...
async function main(): Promise<void> {
  ...
  if (!args.skipBuild) {
    ...
    // Run the same chain as `yarn build:prod` does, inline. Keeping
    // it inline (rather than shelling out to the npm script) means
    // we hit each step's exit code directly — no shell &&-glob.
    await run("yarn", ["tsc", "-b", "--noEmit"]);
    await run("yarn", ["test:unit"]);
    await run("yarn", ["build"]);
    await run("yarn", ["tauri", "build"]);
  } else {

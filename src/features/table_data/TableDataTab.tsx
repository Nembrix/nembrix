...
import {
  ...
  Database, KeyRound, Info, Download, Search,
} from "lucide-react";
...
export default function TableDataTab({ tab }: { tab: Tab }) {
  ...
  return (
    <div className="editor-shell">
      ...
      {/* ─── 3. Record-level toolbar ─── schema.table label, row count,
            search, Limit, Sort, Columns, Export, Refresh. Only really
            applies to the Data view; we still show it on others so the
            schema label + Refresh stay reachable. */}
      <div className="data-view-toolbar">
        <strong>{rel.schema}.{rel.table}</strong>
...
function FragmentRow({
  ...
  const renderTypeCell = () => {
    if (!editingType) {
      return <span>{col.type_name}</span>;
    }
    if (customType) {
      return (
        <input
